...
def tidy_panel(df: pd.DataFrame, years):
    """Reshape wide metric layout to a per-company, per-year panel."""
...
def safe_log(series: pd.Series):
    """Return natural log of positive, non-null entries."""
...
def plot_log_average_ps(data_frames):
    plt.figure(figsize=(10, 5))
    for df, label in data_frames:
        grp = df.groupby("Year")["ValRev"].mean()
        grp = grp.dropna()
        grp = grp[grp > 0]
        plt.plot(grp.index, np.log(grp.values), marker="o", label=label)
...
def plot_peak_boxplots(dotcom_tidy, ai_pure_tidy, ai_niche_tidy):
...
def plot_loglog_mc_vs_rev(combined):
...
def plot_median_bar(dotcom_tidy, ai_pure_tidy, ai_niche_tidy):
    med_data = {
...
def main():
    ...
    # Build tidy datasets for each cohort
    ...
    # Plot exactly the four comparisons described in this script
    plot_log_average_ps(
        [
            (dotcom_tidy, "Dot-com"),
    ...
    plot_peak_boxplots(dotcom_tidy, ai_pure_tidy, ai_niche_tidy)
    combined = pd.concat([dotcom_tidy, ai_pure_tidy, ai_niche_tidy], ignore_index=True)
    plot_loglog_mc_vs_rev(combined)
    plot_median_bar(dotcom_tidy, ai_pure_tidy, ai_niche_tidy)
